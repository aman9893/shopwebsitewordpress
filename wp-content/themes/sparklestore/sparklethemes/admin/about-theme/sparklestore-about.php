	...
	*/
	...
	public static function hide_notices() {
		if ( isset( $_GET['sparklestore-hide-notice'] ) && isset( $_GET['_sparklestore_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_sparklestore_notice_nonce'], 'sparklestore_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'sparklestore' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'sparklestore' ) );
			}
	...
	public function welcome_notice() {
		...
		<div id="message" class="updated sparklestore-message">
			...
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing sparklestore! To fully take advantage of the best our theme can offer please make sure you visit our %swelcome page%s.', 'sparklestore' ), '<a href="' . esc_url( admin_url( 'themes.php?page=sparklestore-welcome' ) ) . '">', '</a>' ); ?></p>
			<p class="submit">
	...
	private function intro() {
		...
		<div class="sparklestore-theme-info">
				<h1>
					...
					<?php printf( '%s', $sparklestore_version ); ?>
				</h1>
						...
						foreach ( $themes->themes as $theme ) {
							if( $active_theme == $theme->name ) { ?>
								...
								<div id="<?php echo $theme->slug; ?>" class="theme active">
									...
									<h3 class="theme-name" id="sparklestore-name"><strong><?php _e( 'Active', 'sparklestore' ); ?></strong>: <?php echo $theme->name; ?></h3>
									<div class="theme-actions">
										<a class="button button-primary customize load-customize hide-if-no-customize" href="<?php echo get_site_url(). '/wp-admin/customize.php' ?>"><?php _e( 'Customize', 'sparklestore' ); ?></a>
									</div>
								</div><!-- .theme active -->
							<?php
							$counter++;
							...
							?>
								<div id="<?php echo $theme->slug; ?>" class="theme">
									...
									<div class="theme-actions">
										...
										?>
											<!-- Install Button -->
											<a data-toggle="tooltip" data-placement="bottom" title="<?php echo 'Downloaded ' . number_format( $theme_details->downloaded ) . ' times'; ?>" class="button button-secondary activate" href="<?php echo esc_url( wp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ); ?>" ><?php _e( 'Install Now', 'sparklestore' ); ?></a>
										<?php } ?>

										<a class="button button-primary load-customize hide-if-no-customize" target="_blank" href="<?php echo $theme->preview_url; ?>"><?php _e( 'Live Preview', 'sparklestore' ); ?></a>
									</div>
								</div><!-- .theme -->
								<?php
							}
