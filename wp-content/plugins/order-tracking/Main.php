...
if ( is_admin() ){
	...
	add_action('init', 'Update_EWD_OTP_Content');
...
function EWD_OTP_Default_Statuses() {
	...
	if ((!is_array($Statuses_Array) or ($Statuses_Array[0]['Status'] == "")) and $StatusString == "") {
		$Save_Statuses_Array = array(
			array("Status" => "Received", "Percentage" => "25", "Message" => "Default", "Internal" => "No"),
			array("Status" => "Processed", "Percentage" => "50", "Message" => "Default", "Internal" => "No"),
			array("Status" => "Shipped", "Percentage" => "75", "Message" => "Default", "Internal" => "No"),
...
function EWD_OTP_Plugin_Menu() {
	...
	if (current_user_can($Access_Role)) {
		add_menu_page('Order Tracking Plugin', 'Tracking', $Access_Role, 'EWD-OTP-options', 'EWD_OTP_Output_Options',null , '50.8');
...
function Add_EWD_OTP_Scripts() {
	if (isset($_GET['page']) && $_GET['page'] == 'EWD-OTP-options') {
		...
		wp_enqueue_script('sorttable', $url_six, array('jquery'));
		$url_one = plugins_url("order-tracking/js/Admin.js");
		wp_enqueue_script('PageSwitch', $url_one, array('jquery', 'sorttable'));
		...
		wp_enqueue_script('specttrum', $url_five, array('jquery'));
	}
}
...
function EWD_OTP_Admin_Options() {
	...
	wp_enqueue_style( 'ewd-otp-spectrum', plugins_url("order-tracking/css/spectrum.css"));
}
